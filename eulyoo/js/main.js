$(document).ready(function(){
         ...
         on: {
            ...
            //   console.log(progress)
              /* 1 -> 0
                ...
                visual_bar_w = visual_bar_w +'%'
                console.log(visual_bar_w)
    ...
    /*****************************************************************
     *    1차 지금 현재 넓이가 pc버전인지 모바일 버전인지 구분(메뉴만 1025px 이상은 pc 1024이하는 모바일)
     *   ==> 브라우저의 넓이값을 구해서 1024보다 큰지 작은지 구분
     *   ==> 첫번째 로딩됐을 때 계산, 그리고 브라우저가 리사이즈 될때마다 브라우저 넓이 체크.
     *     동일한 계산을 두번해야 하는 경우는 함수로 정의한 다음에 호출해서 사용하는 방식
     * 
     * 모바일 메뉴의 규칙
     * 1. 닫힌 메뉴를 클릭하면 열림
     * 2. 오직 하나의 메뉴만 열림
     * 3. 열린 메뉴를 클릭하면 닫힘
     * pc 에서는 1차 메뉴를 클릭하면 첫번째 하위 메뉴로 이동(href 링크 주소가있음)
     * 근데 모바일에서는 1차메뉴를 클릭하면 하위메뉴를 열어줘야한다. (링크 x)
     *  ==>근데 href에 입력된 값은?
     * 반드시 1차 메뉴의 a를 선택자로 해서 href 이벤트를 정지시켜야함
     * ===> 이것 때문에 귀찮아서 1차 메뉴링크를 pc부터 아예 안 하기도 하고 pc/mobile 메뉴를
     *     나눠서 코딩하거나 pc도 클릭하면 하위메뉴가 나오게 함
    ************************************************************************/
    let win_w //브라우저 넓이
    let mobile_size =1024 //모바일 사이즈 시작
    let device_status //pc, mobile 두개의 값 지정 
    ...
    $('.header .uitl .search .search_open').on('focusin' , function(){
          if(device_status == 'pc'){
            $('header').removeClass('menu_over')
        }
    })
    ...
    $('.header .gnb .gnb_wrap ul.depth1 > li > a').on('click' , function(e){
        if(device_status == 'mo'){
            ...
            //내가 클릭한 a를 감싸는 li에 오픈 클래스가 있는지 확인
    ...
    scroll_chk()//함수의 호출 - 문서가 로당되고 단 1번만 실행
    $(window).scroll(function(){
        scroll_chk()//함수의 호출 - 브라우저가 스크롤 될 때마다
    })
